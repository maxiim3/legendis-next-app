...
import {Buttons} from '@/page-section/hero-section/components/buttons';
import NewAlbumWidget from '@/page-section/hero-section/components/NewAlbumWidget';
...
export namespace Containers {
   export function CardSlot({children, className}: ComponentPropsWithoutRef<'div'>) {
      return (
         <div
            className={cn(
               'relative z-[880] flex min-h-[80vh] w-full sm:w-[80vw] flex-col items-center justify-evenly border-8 border-base-100/40 bg-base-100/80 py-2 shadow-[0_4px_20px_8px_rgba(0,0,0,0.7),0_4px_90px_30px_rgba(0,0,0,0.5)] drop-shadow-xl backdrop-blur md:h-auto md:max-h-[95vh] md:max-w-[90vw]  md:gap-2 md:rounded-2xl md:border md:p-8',
               className
            )}>
            {children}
         </div>
