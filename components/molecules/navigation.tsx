...
const useMobileNavigation = create<{
    isOpen: boolean;
...
const MobileNavigation = () => {
    ...
    return (
        <section
            ...
            <label
                className={cn(
                    "swap swap-flip absolute bottom-6 ",
                    isOpen && "swap-active"
                )}
...
const DesktopNavigation = () => (
    <section
        className={
            "hidden md:flex md:items-center md:justify-around md:gap-4 xl:hidden"
        }>
...
const LargeScreenNavigation = () => (
    <aside
        className={
            "hidden xl:flex xl:h-full xl:flex-col xl:items-center xl:justify-around xl:gap-5"
        }>
        <Legendis className={"h-8 w-16 fill-primary"} />
...
const NavigationItems = ({className}: ComponentPropsWithoutRef<"a">) => {
    const {
       sections,
       scrollToSection,
       activeSection} = useNavigationStore(store => store);
    const currentLanguage = useI18nContext(store => store.currentLanguage);

    const {setIsOpen} = useMobileNavigation(store => store);
